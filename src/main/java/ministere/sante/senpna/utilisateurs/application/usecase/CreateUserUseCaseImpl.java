...
public class CreateUserUseCaseImpl implements CreateUserUseCase {
    ...
    public CreatedUser creer(CreateUserCommand command) {
        ...
        if (estCompteFournisseur) {
            ...
            // ces rôles, ex: ADMIN_PNA seul) — lève une exception si
            // l'entrepôt fourni est incohérent avec les rôles demandés.
            entrepotId = entrepotAffectationResolver.resoudre(command.acteurId(), codesRoles, command.entrepotId());
        }
        ...
        // compte dont le rôle exige un entrepôt ou un fournisseur n'existe
        // jamais sans lui.
        if (entrepotId != null) {
            userAffectationRepositoryPort.affecterEntrepot(saved.getId().getValue(), entrepotId);
        } else if (fournisseurId != null) {
            userAffectationRepositoryPort.affecterFournisseur(saved.getId().getValue(), fournisseurId);
        }
